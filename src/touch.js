...
IIPMooViewer.implement({
  ...
  addTouchEvents: function () {
    ...
    if ('ontouchstart' in window || navigator.msMaxTouchPoints) {
      ...
      this.container.addEvent('touchmove', function (e) { 
        e.preventDefault(); 
      });

      // Disable elastic scrolling and handle changes in orientation on 
      // mobile devices. These events need to be added to the document 
      // body itself.
      document.body.addEvents({
        ...
        orientationchange: function () {
          ...
          // Need to set a timeout the div is not resized immediately on some 
          // versions of iOS
      ...
      this.canvas.addEvents({
        touchstart: function (e) {
          ...
          if (e.touches.length == 1) {
            ...
            _this.canvas.setStyle(_this.CSSprefix+'transform', 
              'translateZ(0,0,0)');
          }
        },
        ...
        touchmove: function (e) {
          if (e.touches.length == 1) {
            ...
            if (view_x > _this.wid - _this.view.w) {
              _this.touchend.x = _this.touchstart.x - _this.wid + 
                  _this.view.w + _this.view.x;
            }

            if (view_y > _this.hei - _this.view.h) { 
              _this.touchend.y = _this.touchstart.y - _this.hei + 
                  _this.view.h + _this.view.y;
            }

            if (view_x < 0)  {
               _this.touchend.x = _this.touchstart.x + _this.view.x;
            }

            if (view_y < 0) {
                _this.touchend.y = _this.touchstart.y + _this.view.y;
            }

            left = _this.wid > _this.view.w ? 
                _this.touchend.x - _this.touchstart.x : 0;
            ...
            var xx = Math.round((e.touches[0].pageX + e.touches[1].pageX) / 2) +
                _this.view.x;
        ...
        touchend: function (e) {
          ...
          if (_this.canvas.retrieve('gesturestart') == 1) {
            ...
            var dx = _this.touchend[1].x - _this.touchend[0].x;
            var dy = _this.touchend[1].y - _this.touchend[0].y;
            var d1 = Math.sqrt(dx * dx + dy * dy); 

            dx = _this.touchstart[1].x - _this.touchstart[0].x;
            dy = _this.touchstart[1].y - _this.touchstart[0].y;
            ...
            if (Math.abs(scale) > 20000) {
              ...
              var dx = _this.touchend[1].x - _this.touchend[0].x;
              var dy = _this.touchend[1].y - _this.touchend[0].y;
              var r1 = Math.atan2(dy, dx) * 180 / Math.PI;

              dx = _this.touchstart[1].x - _this.touchstart[0].x;
              dy = _this.touchstart[1].y - _this.touchstart[0].y;
              var r2 = Math.atan2(dy, dx) * 180 / Math.PI;
              ...
              if (Math.abs(rotation) > 25) {
                ...
                _this.rotate(r);
                if(IIPMooViewer.sync) {
                  IIPMooViewer.windows(_this).invoke( 'rotate', r );
                }
              }
            }
            ...
            _this.canvas.setStyles({
              ...
              top: _this.hei > _this.view.h ? 
                  -_this.view.y : Math.round((_this.view.h - _this.hei) / 2) 
            });

            _this.requestImages();
            if (_this.navigation) {
              _this.navigation.update(_this.view.x / _this.wid,
                _this.view.y / _this.hei, 
                _this.view.w / _this.wid, 
                _this.view.h / _this.hei);
            }

            if(IIPMooViewer.sync) {
              IIPMooViewer.windows(_this).invoke('moveTo', 
                _this.view.x, _this.view.y);
            }
...
Class.refactor(Drag, {
  ...
  start: function (event) {
    document.body.addEvents({
      touchmove: 
        this.touchmoveCheck = function (evt) { 
          evt.preventDefault(); 
          this.bound.check(evt); 
        }.bind(this),
